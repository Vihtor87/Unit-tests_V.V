...
public class Main {
    ...
    public static int sum(int a, int b) {
        long res = (long) a + b;
        assert (res <= Integer.MAX_VALUE && res >= Integer.MIN_VALUE) : "Integer overflow!";
        return (int) res;
    }
    ...
    public static void happyNY() {
        ...
        assert currentDateTime.equals("08/04/2024") : "Еще 2023 год :("; //должна быть актуальная дата для проверки!
    ...
    public static void checkingHero() {
        ...
        */
        ...
        assert 50 == emmett.getArmorStrength(): "Armor is not 50";
        assert "sword".equals(emmett.getWeapon()): "Weapon is not sword";
        List<String> expectedBag = Arrays.asList("Bow", "Axe", "Gold");
        assert 3 == expectedBag.size(): "Size`s bag are not 3";
        assert emmett.getBag().contains(expectedBag);
        assert false == expectedBag.isEmpty(): "Array can`t be Empty ";
